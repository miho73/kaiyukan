...
void DrawOverlayContent(HDC hdc, const RECT& clientRect) {
...

  {
	...
	graphics.TranslateTransform(HALF_JELLYFISH_SIZE, HALF_JELLYFISH_SIZE);

	float angle = heading.dot(VECTOR(0, -1));
	angle = acosf(angle) * (180.0f / 3.14159265f);
	if (heading.x < 0) angle = -angle;
	...
	if (
	  ...
	  graphics.DrawImage(
		g_pGifImage,
		-HALF_JELLYFISH_SIZE, -HALF_JELLYFISH_SIZE,
		JELLYFISH_SIZE, JELLYFISH_SIZE
	  );
	}
